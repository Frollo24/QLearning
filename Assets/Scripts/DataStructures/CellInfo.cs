...
namespace Assets.Scripts.DataStructures
{
    public class CellInfo : ICloneable
    {
        ...
        public CellInfo[] GetNeighbours(BoardInfo board)
        {
            var neighbours = new CellInfo[4];
            //UP
            neighbours[0] = (this.RowId < (board.NumRows - 1))
                ? board.CellInfos[this.ColumnId, this.RowId + 1]
                : null;
            //Right
            neighbours[1] = (this.ColumnId < (board.NumColumns - 1))
                ? board.CellInfos[this.ColumnId + 1, this.RowId]
                : null;
            //Down
            neighbours[2] = (this.RowId > 0)
                ? board.CellInfos[this.ColumnId, this.RowId - 1]
                : null;
            //Left
            neighbours[3] = (this.ColumnId > 0)
                ? board.CellInfos[this.ColumnId - 1, this.RowId]
                : null;
            return neighbours;
        }
